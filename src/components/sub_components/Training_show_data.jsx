...
const TrainingDataTable = () => {
  ...
  useEffect(() => {
    axios.get("http://127.0.0.1:8000/api/trainings").then((res) => {
      setData(res.data);
    });
  }, []);
  ...
  const handleSubmit = () => {
    ...
    axios
      .post("http://127.0.0.1:8000/api/trainings", formData)
      .then((res) => {
        setData(res.data);
  ...
  const handleUpdate = () => {
    ...
    axios
      .post(`http://127.0.0.1:8000/api/trainings/${id}`, formData)
      .then((res) => {
        setData(res.data);
  ...
  return (
      ...
      <table className="w-full">
        <thead className="text-xl ">
          ...
          <td className="text-center px-2">Course Name</td>
          <td className="text-center px-2">Header</td>
          <td className="text-center px-2">Description</td>
          <td className="text-center px-2">Image</td>
        </thead>
        <tbody>
          {data.map((item, index) => (
              <tr
